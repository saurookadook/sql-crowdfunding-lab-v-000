...
"SELECT projects.title, SUM(SUM(pledges.amount) - projects.funding_goal) AS funding_amount FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING funding_amount >= 0"
end
