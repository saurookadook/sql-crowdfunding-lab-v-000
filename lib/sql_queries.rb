...
"SELECT projects.title, SUM(pledges.amount) AS total_pledges, SUM(total_pledges - projects.funding_goal) AS funding_amount FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING funding_amount >= 0"
end
