...
"SELECT projects.title, SUM(pledges.amount) - projects.funding_goal AS total_pledges FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING total_pledges >= 0"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(pledges.amount) AS total_pledges FROM users JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY pledges.amount, users.name"
end
