...
"SELECT projects.title, SUM(pledges.amount) AS total_pledges, AS funding_amount FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING (total_pledges - funding_goal) >= 0"
end
